...
namespace ExcelScript.Registration
{
    public class FunctionLoggingHandler : FunctionExecutionHandler
    {
        ...
        public override void OnEntry(FunctionExecutionArgs args)
        {
            ...
            ExcelScriptAddin.m_Log.Debug("{0} - OnEntry - Args: {1}", args.Tag, string.Join(",", args.Arguments.Select(arg => arg.ToString())));
        }

        public override void OnSuccess(FunctionExecutionArgs args)
        {
            ExcelScriptAddin.m_Log.Debug("{0} - OnSuccess - Result: {1}", args.Tag, args.ReturnValue);
        }

        public override void OnException(FunctionExecutionArgs args)
        {
            ExcelScriptAddin.m_Log.Error("{0} - OnException - Message: {1}", args.Tag, args.Exception);
        }

        public override void OnExit(FunctionExecutionArgs args)
        {
            ExcelScriptAddin.m_Log.Debug("{0} - OnExit", args.Tag);
        }
